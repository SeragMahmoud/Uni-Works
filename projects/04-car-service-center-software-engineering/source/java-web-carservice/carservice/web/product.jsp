...
paymentHtml += '<button id="payCash">Pay with Cash</button>';
paymentHtml += '<button id="payCard">Pay with Card</button>';


    cartWindow.document.body.innerHTML = paymentHtml;

    setTimeout(function() {
        cartWindow.document.getElementById("payCash").addEventListener("click", payWithCash);
        cartWindow.document.getElementById("payCard").addEventListener("click", payWithCard);
    }, 100);
}

// Cash Payment Modal (open a new window with success message)
// Cash Payment Modal (open a new window with success message)
